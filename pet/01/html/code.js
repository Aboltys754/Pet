...
async function submit_contact() {
    ...
    let respon = await fetch(`http://localhost:3000/addContact`, {
        method: 'POST',
...
async function editContact() {
    ...
    let respon = await fetch('http://localhost:3000/editContact', {
        method: 'PATCH',
        body: JSON.stringify({
            id: activAtrribute[1], 
            name: name.value, 
...
async function delet_contact(event) {
    ...
    if (confirm('Вы точно хотите удалить контакт?') === true) {
        let respon = await fetch(`http://localhost:3000/deleteContact/${elemId}`, {
        method: 'DELETE',
    });
